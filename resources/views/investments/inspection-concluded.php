<?php $pageTitle = 'Project Concluded'; ?>
...
<div class="container-fluid py-4">
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-3">
                        <i class="fas fa-filter me-2"></i>Filter Project Concluded Projects
                    </h6>
            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6>Project Concluded Projects</h6>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 table-hover">
                            ...
                            <tbody>
                                <?php if (!empty($investmentconcludedList['data'])): ?>
                                    <?php foreach ($investmentconcludedList['data'] as $project): ?>
                                        <tr>
                                                ...
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>">
                                                        ...
                                                        <?php if (($project['itemstatusid'] ?? 0) != 4): ?>
                                                        <li>
                                                            <a class="dropdown-item approve-project" href="javascript:;" 
                                                        ...
                                                        <?php endif; ?>
                                                        <?php if (($project['itemstatusid'] ?? 0) != 3): ?>
                                                        <li>
                                                            <a class="dropdown-item reinspect-project" href="javascript:;" 
                                                               data-itemid="<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>"
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                                                <i class="fas fa-redo me-2"></i>Reinspect
                                                            </a>
                                                        </li>
                                                        <?php endif; ?>
                                                        <?php if (($project['itemstatusid'] ?? 0) != 5): ?>
                                                        <li>
                                                            <a class="dropdown-item text-warning reject-project" href="javascript:;" 
                                                               data-itemid="<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>"
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="10" class="text-center py-4">
                                            <p class="text-sm text-secondary mb-0">No Project Concluded projects found</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php $paginationData = $investmentconcludedList; include __DIR__ . '/../partials/pagination.php'; ?>
...
(function(){
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', function(){
            ...
            window.location.href = (() => {
                const baseUrl = '<?php echo url("investments/inspection-concluded"); ?>';
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', function(){
            window.location.href = '<?php echo url("investments/inspection-concluded"); ?>';
        });
    }
})();
...
document.addEventListener('click', function(e){
    // Approve handler
    const approve = e.target.closest('.approve-project');
    if (approve) {
        e.preventDefault();
        const id = approve.getAttribute('data-itemid');
        const title = approve.getAttribute('data-title') || 'this project';
        if (!id) return;
        if (!confirm(`Are you sure you want to approve "${title}"?`)) return;
        
        const formData = new FormData();
        formData.append('itemstatusid', '4');
        formData.append('inspectionstatusid', '4');
        
        fetch(`/investments/projects/update/${id}`, { 
            method: 'POST',
            body: formData
        })
        .then(r => r.json()).then(d => {
            if (d.success) {
                ...
                alert(d.message || 'Failed to approve project');
            }
        }).catch(() => alert('Error while approving project'));
        return;
    }
    
    // Reinspect handler
    const reinspect = e.target.closest('.reinspect-project');
    if (reinspect) {
        e.preventDefault();
        const id = reinspect.getAttribute('data-itemid');
        const title = reinspect.getAttribute('data-title') || 'this project';
        if (!id) return;
        if (!confirm(`Are you sure you want to set "${title}" for reinspection?`)) return;
        
        const formData = new FormData();
        formData.append('itemstatusid', '3');
        formData.append('inspectionstatusid', '3');
        ...
        .then(r => r.json()).then(d => {
            if (d.success) {
                alert('Project set for reinspection successfully');
                window.location.reload();
            } else {
                alert(d.message || 'Failed to set project for reinspection');
            }
        }).catch(() => alert('Error while setting project for reinspection'));
        return;
    }
    
    // Reject handler
    const reject = e.target.closest('.reject-project');
    if (reject) {
        e.preventDefault();
        const id = reject.getAttribute('data-itemid');
        const title = reject.getAttribute('data-title') || 'this project';
        if (!id) return;
        if (!confirm(`Are you sure you want to reject "${title}"?`)) return;
        
        const formData = new FormData();
        formData.append('itemstatusid', '5');
        formData.append('inspectionstatusid', '5');
        ...
        .then(r => r.json()).then(d => {
            if (d.success) {
                ...
                alert(d.message || 'Failed to reject project');
            }
        }).catch(() => alert('Error while rejecting project'));
        return;
    }
});
</script>
