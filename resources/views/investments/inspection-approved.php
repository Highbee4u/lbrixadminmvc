...
<div class="container-fluid py-4">
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-3">
                        <i class="fas fa-filter me-2"></i>Filter Inspection Approved Projects
                    </h6>
            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6>Inspection Approved Projects</h6>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 table-hover">
                            ...
                            <tbody>
                                <?php if (!empty($inspectionApprovedList['data'])): ?>
                                    <?php foreach ($inspectionApprovedList['data'] as $project): ?>
                                        <tr>
                                             <td class="align-middle text-center">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>">
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item reject-project" href="javascript:;" 
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item reinspect-project" href="javascript:;" 
                                                               data-itemid="<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>"
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                                                <i class="fas fa-redo me-2"></i>Re-Inspect
                                                            </a>
                                                        </li>
                                                        <li>
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item keep-listing-project" href="javascript:;" 
                                                               data-itemid="<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>"
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="10" class="text-center py-4">
                                            <p class="text-sm text-secondary mb-0">No Inspection Approved projects found</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php $paginationData = $inspectionApprovedList; include __DIR__ . '/../partials/pagination.php'; ?>
...
// Reject project handler
document.addEventListener('click', function(e){
    const rejectBtn = e.target.closest('.reject-project');
    if (rejectBtn) {
        const id = rejectBtn.getAttribute('data-itemid');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(({ok, data}) => {
                    if (ok && data.success) {
                        toastr.success('Project rejected successfully');
...
// Reinspect project handler
document.addEventListener('click', function(e){
    const reinspectBtn = e.target.closest('.reinspect-project');
    if (reinspectBtn) {
        const id = reinspectBtn.getAttribute('data-itemid');
        const title = reinspectBtn.getAttribute('data-title');
        if (!id) {
            toastr.error('Project ID is required');
            return;
        }
        ...
        Swal.fire({
            ...
            text: `Are you sure you want to send "${title}" for re-inspection?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#ffc107',
            cancelButtonColor: '#6c757d',
            confirmButtonText: 'Yes, re-inspect it!'
        }).then((result) => {
            if (result.isConfirmed) {
                const formData = new FormData();
                ...
                .then(({ok, data}) => {
                    if (ok && data.success) {
                        toastr.success('Project sent for re-inspection successfully');
                        setTimeout(() => window.location.reload(), 1500);
                    } else {
                        toastr.error(data.message || 'Failed to send project for re-inspection');
                    }
                }).catch(() => toastr.error('Error occurred while sending project for re-inspection'));
            }
        });
    }
});
...
document.addEventListener('click', function(e){
    const listBtn = e.target.closest('.list-project');
    if (listBtn) {
        const id = listBtn.getAttribute('data-itemid');
        const title = listBtn.getAttribute('data-title');
        if (!id) {
            toastr.error('Project ID is required');
            return;
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                }).catch(() => toastr.error('Error occurred while listing project'));
            }
        });
    }
});

// Keep for Listing handler
document.addEventListener('click', function(e){
    const keepBtn = e.target.closest('.keep-listing-project');
    if (keepBtn) {
        const id = keepBtn.getAttribute('data-itemid');
        const title = keepBtn.getAttribute('data-title');
        if (!id) {
            toastr.error('Project ID is required');
            return;
        }
        ...
        Swal.fire({
            title: 'Mark for Listing?',
            text: `Are you sure you want to mark "${title}" for listing?`,
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                .then(({ok, data}) => {
                    if (ok && data.success) {
                        toastr.success('Project marked for listing successfully');
                        setTimeout(() => window.location.reload(), 1500);
                    } else {
                        toastr.error(data.message || 'Failed to mark project for listing');
                    }
                }).catch(() => toastr.error('Error occurred while marking project for listing'));
            }
        });
    }
});
