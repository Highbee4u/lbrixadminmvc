...
<div class="container-fluid py-4">
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-3">
                        <i class="fas fa-filter me-2"></i>Filter Listed Projects
                    </h6>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 table-hover">
                            ...
                            <tbody>
                                <?php if (!empty($listedInvestmentList['data'])): ?>
                                    <?php foreach ($listedInvestmentList['data'] as $project): ?>
                                        <tr>
                                            <td class="align-middle text-center">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>">
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item close-offer" href="javascript:;" 
                                                               ...
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                                                <i class="fas fa-ban me-2"></i>Close Offer
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item text-warning de-list" href="javascript:;" 
                                                               data-itemid="<?php echo htmlspecialchars($project['itemid'] ?? '', ENT_QUOTES); ?>"
                                                               data-title="<?php echo htmlspecialchars($project['title'] ?? '', ENT_QUOTES); ?>">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="10" class="text-center py-4">
                                            <p class="text-sm text-secondary mb-0">No Listed projects found</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php $paginationData = $listedInvestmentList; include __DIR__ . '/../partials/pagination.php'; ?>
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('close-offer') || e.target.closest('.close-offer')) {
        e.preventDefault();
        const element = e.target.closest('.close-offer');
        ...
        Swal.fire({
            title: 'Close Offer?',
            text: `Are you sure you want to close the offer for "${title}"?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                .then(({ok, data}) => {
                    if (ok && data.success) {
                        toastr.success('Offer closed successfully');
...
// Handle de-list
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('de-list') || e.target.closest('.de-list')) {
        e.preventDefault();
        const element = e.target.closest('.de-list');
        const itemid = element.dataset.itemid;
        const title = element.dataset.title;
        ...
        Swal.fire({
            title: 'De-list Project?',
            text: `Are you sure you want to de-list "${title}"?`,
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                .then(({ok, data}) => {
                    if (ok && data.success) {
                        toastr.success('Project de-listed successfully');
                        setTimeout(() => window.location.reload(), 1500);
                    } else {
                        toastr.error(data.message || 'Failed to de-list project');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    toastr.error('An error occurred while de-listing the project');
                });
            }
        });
    }
