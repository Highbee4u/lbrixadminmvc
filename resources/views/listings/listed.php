<?php $pageTitle = 'Listed Properties'; ?>
...
<div class="container-fluid py-4">
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-3">
                        <i class="fas fa-filter me-2"></i>Filter Listed Properties
                    </h6>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 table-hover">
                            ...
                            <tbody>
                                <?php if (!empty($listedProperties['data']) && count($listedProperties['data']) > 0): ?>
                                    <?php foreach ($listedProperties['data'] as $property): ?>
                                        <tr>
                                            <td class="align-middle text-center">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown<?php echo $property['itemid']; ?>">
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item text-warning close-offer" href="javascript:;" data-itemid="<?php echo $property['itemid']; ?>" data-title="<?php echo htmlspecialchars($property['title']); ?>">
                                                                <i class="fas fa-times-circle me-2"></i>Close Offer
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item text-danger delist-property" href="javascript:;" data-itemid="<?php echo $property['itemid']; ?>" data-title="<?php echo htmlspecialchars($property['title']); ?>">
                                                                <i class="fas fa-minus-circle me-2"></i>De-List
                                                            </a>
                                                        </li>
                                                    </ul>
                                                </div>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="9" class="text-center py-4">
                                            <p class="text-sm text-secondary mb-0">No Listed properties found</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php $paginationData = $listedProperties; include __DIR__ . '/../partials/pagination.php'; ?>
...
$(document).ready(function() {
    ...
    // Handle close offer action
    $(document).on('click', '.close-offer', function(e) {
        e.preventDefault();
        ...
        Swal.fire({
            title: 'Close Offer?',
            html: `Are you sure you want to close the offer for <strong>"${title}"</strong>?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                fetch('<?php echo url("listings/properties/close-offer"); ?>/' + itemid, {
                    method: 'POST'
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        toastr.success(data.message || 'Offer has been closed successfully.');
                        setTimeout(() => location.reload(), 1000);
                    } else {
                        toastr.error(data.message || 'Failed to close offer.');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    toastr.error('An error occurred while closing the offer.');
                });
            }
        });
    });

    // Handle delist property action
    $(document).on('click', '.delist-property', function(e) {
        e.preventDefault();
        const itemid = $(this).data('itemid');
        const title = $(this).data('title');
        ...
        Swal.fire({
            ...
            html: `Are you sure you want to de-list <strong>"${title}"</strong>?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
            cancelButtonColor: '#6c757d',
            confirmButtonText: 'Yes, De-list It',
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                fetch('<?php echo url("listings/properties/delist"); ?>/' + itemid, {
                    method: 'POST'
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        toastr.success(data.message || 'Property has been de-listed successfully.');
                        setTimeout(() => location.reload(), 1000);
                    } else {
                        toastr.error(data.message || 'Failed to de-list property.');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    toastr.error('An error occurred while de-listing the property.');
                });
            }
        });
    });
