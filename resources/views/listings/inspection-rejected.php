<?php $pageTitle = 'Inspection Rejected'; ?>
...
<div class="container-fluid py-4">
 <!-- Filter Bar Section -->
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-3">
                        <i class="fas fa-filter me-2"></i>Filter Rejected Properties
                    </h6>
                ...
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="<?php echo url('listings/add-property'); ?>" class="btn btn-success">
                            <i class="fas fa-plus me-2"></i>Add Rejected Property
                        </a>
                    </div>
                </div>
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 table-hover">
                            ...
                            <tbody>
                                <?php if (!empty($rejectedInspections['data']) && count($rejectedInspections['data']) > 0): ?>
                                    <?php foreach ($rejectedInspections['data'] as $property): ?>
                                        <tr>
                                            <td class="align-middle text-center">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown<?php echo $property['itemid']; ?>">
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item text-success approve-property" href="javascript:;" data-itemid="<?php echo $property['itemid']; ?>" data-title="<?php echo htmlspecialchars($property['title']); ?>">
                                                                <i class="fas fa-check me-2"></i>Approve
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item text-info reinspect-property" href="javascript:;" data-itemid="<?php echo $property['itemid']; ?>" data-title="<?php echo htmlspecialchars($property['title']); ?>">
                                                                <i class="fas fa-search me-2"></i>Reinspect
                                                            </a>
                                                        </li>
                                                    </ul>
                                                </div>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="9" class="text-center py-4">
                                            <p class="text-sm text-secondary mb-0">No Rejected Properties found</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php $paginationData = $rejectedInspections; include __DIR__ . '/../partials/pagination.php'; ?>
...
<script>
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('approve-property') || e.target.closest('.approve-property')) {
            e.preventDefault();
            const itemid = e.target.closest('.approve-property').dataset.itemid;
            const title = e.target.closest('.approve-property').dataset.title;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.success) {
                            toastr.success(data.message || 'Property approved successfully');
                            setTimeout(() => { location.reload(); }, 1000);
                        } else {
                            toastr.error(data.message || 'Failed to approve property');
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        toastr.error('An error occurred while approving the property');
                    });
                }
            });
        }
    ...
    // Handle reinspect property
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('reinspect-property') || e.target.closest('.reinspect-property')) {
            e.preventDefault();
            const itemid = e.target.closest('.reinspect-property').dataset.itemid;
            const title = e.target.closest('.reinspect-property').dataset.title;

            Swal.fire({
                title: 'Reinspect Property?',
                text: `Are you sure you want to send "${title}" back for reinspection?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#17a2b8',
                cancelButtonColor: '#6c757d',
                confirmButtonText: 'Yes, reinspect it!',
                cancelButtonText: 'Cancel'
            }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.success) {
                            toastr.success(data.message || 'Property marked for reinspection successfully');
                            setTimeout(() => { location.reload(); }, 1000);
                        } else {
                            toastr.error(data.message || 'Failed to mark property for reinspection');
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        toastr.error('An error occurred while marking property for reinspection');
                    });
                }
            });
        }
